...
class OccupancyDetector:
    ...
    def detect_obstacles_from_point_cloud(self,
        ...
        if gripper_position is not None and len(ignore_ids) > 0:
            ...
            # 🔥 关键修复：增大过滤半径到25cm，确保被抓取的物品不会被检测为障碍物
            # 机械臂抓取的物品可能比较大，15cm可能不够
            gripper_radius = 0.25  # 从0.15增加到0.25
            is_not_near_gripper = distances_to_gripper > gripper_radius
            
            # 统计被过滤的点数
            num_filtered_gripper = np.sum(~is_not_near_gripper & valid_mask)
            if num_filtered_gripper > 0:
                # 每10次感知才打印一次，避免刷屏
                if np.random.random() < 0.1:
                    print(f"  [深度感知] 🔥 过滤夹爪{gripper_radius*100:.0f}cm范围内的 {num_filtered_gripper} 个点（被抓取物品）")

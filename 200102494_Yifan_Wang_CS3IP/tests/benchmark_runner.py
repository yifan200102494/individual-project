...
class BenchmarkConfig:
    ...
    gap_widths_m is defined as the actual clearance width between the inner
    surfaces of the two pillars, not the center-to-center distance.
    """
    gap_widths_m: tuple = (0.40, 0.35, 0.30, 0.25, 0.20)
    n_trials_per_gap: int = 30
    timeout_sec: float = 45.0

    # Pillar half-extents (x, y, z)
    pillar_half_extents: tuple = (0.05, 0.05, 0.20)

    # Gate center position
    gate_center: tuple = (0.50, 0.10, 0.20)

    # Task-success criterion: tolerance from cube's final xy to tray center
    final_xy_tolerance_m: float = 0.15

    # Add a small jitter to the cube's initial xy each trial, to avoid full determinism.
    # Set to 0.0 to reproduce experiments exactly.
...
class BenchmarkFailure(RuntimeError):
    """Internal exception used in sim_step_callback to abort the benchmark early."""


def setup_benchmark_env_headless(cube_xy_jitter=(0.0, 0.0)):
    """Create a headless benchmark environment."""
    if p.isConnected():
        p.disconnect()
...
def create_narrow_passage(clear_gap_width_m, gate_center, pillar_half_extents):
    """
    Create two pillars that form a narrow passage.

    clear_gap_width_m = the actual clearance width between the pillars' inner surfaces.
    """
...
def evaluate_final_task_state(cube_id, tray_id, xy_tolerance_m):
    """Evaluate whether the final placement succeeded."""
...
def classify_failure(metrics, final_state):
    """Classify the failure reason using the paper's convention."""
    if metrics["timed_out"]:
        return "Timeout"
    if metrics["total_collision_frames"] > 0:
        return "Collision"
...
def run_single_test(task):
    ...
    try:
        ...
        def benchmark_monitor():
            ...
            # 1) Only count contacts between robot/cube and the benchmark pillars
            hit_gate = False
            ...
            # 2) Record the minimum signed clearance from robot + cube to each pillar
            current_clearances = []
            for pillar_id in pillar_ids:
            ...
            # 3) Keep the original benchmark's "glue cube" logic so this experiment doesn't turn into a grasp-stability test
...
def main():
    ...
    print(f"Starting 5.2.4 benchmark: {len(tasks)} runs, {max_workers} workers")

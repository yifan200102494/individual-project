...
def run_robustness_test(save_dir=None, show_plot=False):
    if save_dir is None:
    ...
    if reduction > 30:
        print("[Pass] KF successfully smoothed out the visual noise, proving the value of high-frequency prediction.")

    # 创建保存目录
    os.makedirs(save_dir, exist_ok=True)

    csv_path = os.path.join(save_dir, "figure_6_2_3_kalman_noisy_observations.csv")
    ...
    out_path = os.path.join(save_dir, "figure_6_2_3_kalman_noisy_observations.png")
...
if __name__ == "__main__":
    run_robustness_test(show_plot=False)
